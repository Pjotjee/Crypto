...
/*
Sends a request to cryptocompare to retrieve the historical values of the coin.
 */
class DataRequest implements Response.Listener<JSONObject>, Response.ErrorListener {
    private Context context;
    private Callback activity;                                 
    private ArrayList<Double> dataLow = new ArrayList<Double>();
    private ArrayList<Double> dataHigh = new ArrayList<Double>();
    private ArrayList<Double> dataTime = new ArrayList<Double>();
    private ArrayList<Double> dataOpen = new ArrayList<Double>();
    private ArrayList<Double> dataClose = new ArrayList<Double>();
    ...
    * that instantiated the request that it is done through the callback.*/
    void getData(Callback activity, String symbol) {
        // saves activity as instance variable
        this.activity = activity;
        // create new request queue for API requests
        RequestQueue queue = Volley.newRequestQueue(context);
        String url = "https://min-api.cryptocompare.com/data/histominute?fsym=" + symbol + "&tsym=USD&limit=60";
